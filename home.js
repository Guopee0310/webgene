...
window.addEventListener('scroll', function() {
    ...
    if (scrollY > header.clientHeight*6) {
...
navLinks.forEach(link => {
  link.addEventListener('click', event => {
    ...
    const targetId = link.getAttribute('href').substring(1); // 获取目标 section 的 id
    const targetSection = document.getElementById(targetId); // 获取目标 section 元素

    if (targetSection) {
      // 使用 scrollIntoView 方法滚动到目标 section
